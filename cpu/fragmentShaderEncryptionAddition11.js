...
document.write('\
        <script id="fragmentShaderEncryptionAddition11" type="x-shader/x-fragment">\
			...
			uniform float message[11];\
					...
					void main() {\
						...
						vec4 inputTextureAtThisPixel = texture2D(sTexture,vec2(gl_FragCoord.x/11.0,0.5));\
						gl_FragColor.r = inputTextureAtThisPixel.r;\
						for(int i = 0; i < 11; i++) {\
							if(coordInt == i) {\
								gl_FragColor.r = inputTextureAtThisPixel.r;\
								gl_FragColor.g += float(int(floor(modulus(float(inputTextureAtThisPixel.r + message[i]),float(32)))));\
